...
export const TopNavBar = ({ onCommandPalette, onShortcutsModal, onMenuClick }: TopNavBarProps) => {
  const { isMobile } = useResponsive();

  return (
    <div className="sticky top-0 left-0 right-0 z-40 h-14 sm:h-16 border-b border-neutral-200 bg-white/95 backdrop-blur-sm flex items-center justify-between px-3 sm:px-4 lg:px-6 gap-2 sm:gap-4">
      ...
      {onCommandPalette && (
        <>
          <button
            ...
            className="hidden sm:flex items-center gap-3 px-3 sm:px-4 py-2 sm:py-2.5 bg-neutral-50 hover:bg-neutral-100 border border-neutral-200 rounded-lg transition-all hover:shadow-sm max-w-md w-full"
          >
