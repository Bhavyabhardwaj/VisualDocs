  ...
  const handleGenerateDocumentation = async () => {
    if (!id || !analysis) {
      toast({
        ...
        description: 'Please run analysis first before generating documentation.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: 'Creating markdown documentation from analysis...',
      });

      // Generate markdown documentation from analysis data
      const markdown = `# ${project?.name || 'Project'} Documentation

## Overview
${project?.description || 'No description provided'}

## Project Metrics
- **Total Files**: ${(analysis as any).totalFiles || 0}
- **Lines of Code**: ${(analysis as any).totalLinesOfCode?.toLocaleString() || 0}
- **Functions**: ${(analysis as any).functionCount || 0}
- **Classes**: ${(analysis as any).classCount || 0}
- **Interfaces**: ${(analysis as any).interfaceCount || 0}

## Complexity Analysis
${(analysis as any).complexity ? `
- **Total Complexity**: ${(analysis as any).complexity.total}
- **Average Complexity**: ${(analysis as any).complexity.average?.toFixed(2)}
` : 'No complexity data available'}

## Language Distribution
${(analysis as any).languageDistribution ? Object.entries((analysis as any).languageDistribution)
  .map(([lang, count]) => `- **${lang.charAt(0).toUpperCase() + lang.slice(1)}**: ${count} files`)
  .join('\n') : 'No language data available'}

## Dependencies

### External Dependencies (${(analysis as any).dependencies?.external?.length || 0})
${(analysis as any).dependencies?.external?.length > 0 ? 
  (analysis as any).dependencies.external.map((dep: string) => `- \`${dep}\``).join('\n') : 
  'No external dependencies found'}

### Internal Dependencies
- **Count**: ${(analysis as any).dependencies?.internal?.length || 0}

## AI Recommendations
${(analysis as any).recommendations?.length > 0 ? 
  (analysis as any).recommendations.map((rec: string, idx: number) => `${idx + 1}. ${rec}`).join('\n') : 
  'No recommendations available'}

## Analysis Details
- **Analysis ID**: ${(analysis as any).id}
- **Completed At**: ${(analysis as any).completedAt ? new Date((analysis as any).completedAt).toLocaleString() : 'N/A'}
...
`;
      ...
      toast({
        ...
        description: 'View your documentation in the Documentation tab.',
      });
    } catch (error) {
                  ...
                  <CardContent>
                    {documentation ? (
                      ...
                      <div className="text-center py-8">
                        ...
                        <p className="text-sm text-neutral-600 mb-4">
                          {!analysis ? 
                            'Run analysis first to generate documentation' : 
