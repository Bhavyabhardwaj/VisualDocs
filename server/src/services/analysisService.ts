...
export class AnalysisService {
    ...
    async analyzeProject(projectId: string, userId: string, fileIds?: string[]): Promise<ProjectAnalysisResult> {
        try {
            ...
            // Auto-generate diagrams after successful analysis
            try {
                logger.info('Auto-generating diagrams', { projectId });
                await this.autoGenerateDiagrams(projectId, userId, files);
            } catch (diagramError) {
                logger.warn('Diagram auto-generation failed, continuing', {
                    projectId,
                    error: diagramError instanceof Error ? diagramError.message : 'Unknown'
                });
            }
    ...
    async generateDocumentation(projectId: string, userId: string): Promise<string> {
        try {
            logger.info('Generating AI-powered documentation', { projectId, userId });
            ...
            const aiDocumentation = await aiService.generateComprehensiveDocumentation(
                project.name,
                project.description || '',
                aggregated,
                files
            ...
            // Combine AI documentation with technical metrics
            const fullDocumentation = this.buildFullDocumentation(project, aggregated, files, aiDocumentation);

            logger.info('Documentation generated successfully', { 
                projectId, 
                documentationLength: fullDocumentation.length 
            });
    ...
    private buildFullDocumentation(
        project: any,
        ...
        // Title
        doc.push(`# ${project.name} - Complete Documentation\n\n`);
        doc.push(`> ${project.description || 'Comprehensive AI-generated project documentation'}\n\n`);
        ...
        doc.push(`## 📊 Technical Metrics\n\n`);
        doc.push(`### Code Statistics\n\n`);
        doc.push(`| Metric | Value |\n`);
        doc.push(`|--------|-------|\n`);
        doc.push(`| Total Files | ${analysis.totalFiles} |\n`);
        doc.push(`| Lines of Code | ${analysis.totalLinesOfCode.toLocaleString()} |\n`);
        doc.push(`| Functions | ${analysis.functionCount} |\n`);
        doc.push(`| Classes | ${analysis.classCount} |\n`);
        doc.push(`| Interfaces | ${analysis.interfaceCount} |\n`);
        doc.push(`| Avg Complexity | ${analysis.complexity.average.toFixed(2)} |\n`);
        ...
        // Footer
        doc.push(`---\n\n`);
        doc.push(`*Generated by VisualDocs AI - ${new Date().toLocaleString()}*\n`);
    ...
    private async autoGenerateDiagrams(
        projectId: string, 
        userId: string, 
        files: any[]
    ): Promise<void> {
        try {
            ...
            if (!project) {
                logger.error('Project not found for diagram generation', { projectId });
                return;
            }
